...
func TestDefinitionInputsIgnoreGeneratedPythonCaches(t *testing.T) {
	root := fixtureRepo(t)
	cachePath := filepath.Join(root, "scanners", "ci", "__pycache__", "tool.cpython-314.pyc")
	if err := os.MkdirAll(filepath.Dir(cachePath), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(cachePath, []byte("generated bytecode"), 0o644); err != nil {
		t.Fatal(err)
	}
	lock, err := Generate(context.Background(), root, GenerateOptions{})
	if err != nil {
		t.Fatal(err)
	}
	if _, exists := lock.Definition.Inputs["scanners/ci/__pycache__/tool.cpython-314.pyc"]; exists {
		t.Fatal("generated Python bytecode was included in definition inputs")
	}
}
