...
func (s *PostgresStore) CreateRemoteNode(ctx context.Context, node *models.RemoteNode) error {
	now := time.Now().UTC()
	node.CreatedAt = now
	node.UpdatedAt = now
	if node.Port == 0 {
...
func (s *PostgresStore) GetRemoteNodeByID(ctx context.Context, id string) (*models.RemoteNode, error) {
	var node models.RemoteNode
	err := s.db.GetContext(ctx, &node, "SELECT * FROM remote_nodes WHERE id = $1", id)
	if err != nil {
		return nil, err
	}
	return &node, nil
...
func (s *PostgresStore) UpdateRemoteNode(ctx context.Context, node *models.RemoteNode) error {
...
func (s *PostgresStore) DeleteRemoteNode(ctx context.Context, id string) error {
	_, err := s.db.ExecContext(ctx, "DELETE FROM remote_nodes WHERE id = $1", id)
	return err
}

func (s *PostgresStore) TouchRemoteNodeCheck(ctx context.Context, id, status, checkError string) error {
	now := time.Now().UTC()
	_, err := s.db.ExecContext(ctx,
		`UPDATE remote_nodes SET last_check_status = $1, last_check_error = $2, last_checked_at = $3, updated_at = $4 WHERE id = $5`,
