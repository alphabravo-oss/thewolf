...
func (s *SQLiteStore) UpsertQualityPolicy(ctx context.Context, policy *models.QualityPolicy) error {
	now := time.Now().UTC()
...
func (s *SQLiteStore) GetQualityPolicyByID(ctx context.Context, id string) (*models.QualityPolicy, error) {
	var policy models.QualityPolicy
	err := s.db.GetContext(ctx, &policy, "SELECT * FROM quality_policies WHERE id = ?", id)
	if err != nil {
		return nil, err
	}
	return &policy, nil
...
func (s *SQLiteStore) ListQualityPolicies(ctx context.Context, scope, scopeID string) ([]models.QualityPolicy, error) {
	...
	if scope == "" {
		...
		err = s.db.SelectContext(ctx, &policies,
			"SELECT * FROM quality_policies WHERE scope = ? AND scope_id = ? ORDER BY name", scope, scopeID)
	}
	return policies, err
}

func (s *SQLiteStore) UpsertQualityGateResult(ctx context.Context, result *models.QualityGateResult) error {
	now := time.Now().UTC()
...
func (s *SQLiteStore) GetQualityGateResult(ctx context.Context, scanID, policyID string) (*models.QualityGateResult, error) {
	var result models.QualityGateResult
	err := s.db.GetContext(ctx, &result,
		"SELECT * FROM quality_gate_results WHERE scan_id = ? AND policy_id = ?", scanID, policyID)
	if err != nil {
		return nil, err
	}
	return &result, nil
...
func (s *SQLiteStore) ListQualityGateResults(ctx context.Context, scanID string) ([]models.QualityGateResult, error) {
	var results []models.QualityGateResult
	err := s.db.SelectContext(ctx, &results,
		"SELECT * FROM quality_gate_results WHERE scan_id = ? ORDER BY evaluated_at DESC", scanID)
	return results, err
}
